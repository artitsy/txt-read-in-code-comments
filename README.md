...
可以在插件设置 `AutoHide` 设置，有三种自动隐藏策略：
- 计时自动隐藏。默为 10 秒。设置为0表示禁用。
- 保存时自动隐藏。默认未开启。
  - 显示位置为行内注释时，文件保存前会隐藏该文件中的文本。
  - 显示位置为状态栏时，任意文件的保存会隐藏状态栏中的文本。
  
  如开启，请留意您的 `files.autoSave` 设置。
- 切换窗口自动隐藏。默认未开启。当焦点离开 vscode 窗口时触发。
